...
$sql = "SELECT * FROM jobs where job_name like '%$jobName%' and c_name like '%$companyName%' and status='1' LIMIT $start_from, $num_rec_per_page";
...
if($result->rowCount()==0){
  ...
  $sql="select count(*) as shuliang from jobs where job_name like '%$jobName%' and c_name like '%$companyName%' and status='1'";
  $result=$db->query($sql);
  $row=$result->fetch(1);
